...
export default function ThemePicker({ themes }: ThemePickerProps) {
  ...
  const currentThemeId = pathneme.startsWith("/theme/")
    ? pathneme.split("/theme/")[1]
    : "";
  ...
  return (
    <div className="theme-bar flex gap-6">
      <Link href={`/`}>
        <Button
          variant="ghost"
          className={`theme-item p-[10px] text-sm font-bold cursor-pointer transition-all duration-500 hover:text-sky-700 ${
